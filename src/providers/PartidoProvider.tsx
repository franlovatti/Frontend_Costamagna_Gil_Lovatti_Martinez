...
const PartidoProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const cargarResultado = useCallback(async (resultado: resultadosDto) => {
    ...
    try {
      const response = await apiAxios.put(`/partidos/${resultado.partidoId}`, {
        resultadoLocal:
          resultado.resultadoLocal === ''
            ? null
            : Number(resultado.resultadoLocal),
        resultadoVisitante:
          resultado.resultadoVisitante === ''
            ? null
            : Number(resultado.resultadoVisitante),
      });
      return response.data.data;
    } catch (err) {
      const axiosError = err as AxiosError<{ message?: string }>;
  ...
  const editarPartido = useCallback(
    async (partidoId: number, payload: PartidoPayload) => {
      setLoading(true);
      setError(null);

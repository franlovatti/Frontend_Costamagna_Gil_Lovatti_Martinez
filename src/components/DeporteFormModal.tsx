    ...
    <div className="modal-overlay" onClick={() => setShowModal(false)}>
      <div className="modal-content-custom" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mb-3">
          ...
          <input
            type="number"
            className={`form-control custom-input ${errors.cantMinJugadores ? 'is-invalid' : ''}`}
          ...
          {errors.cantMinJugadores && (
            <span className="auth-error-text">{errors.cantMinJugadores.message}</span>
          )}
        </div>
        ...
        <div className="mb-3">
          ...
          <input
            type="number"
            className={`form-control custom-input ${errors.cantMaxJugadores ? 'is-invalid' : ''}`}
            {...register("cantMaxJugadores", {
              ...
              validate: value => {
                return value >= minimo || "La cantidad máxima debe ser mayor o igual a la mínima";
              },
              valueAsNumber: true
            })}
          />
          {errors.cantMaxJugadores && (
            <span className="auth-error-text">{errors.cantMaxJugadores.message}</span>
          )}
        </div>
        ...
        <div className="mb-3">
          ...
          <input
            type="number"
            className={`form-control custom-input ${errors.duracion ? 'is-invalid' : ''}`}
            {...register("duracion", { 
        ...
        <div className="d-flex gap-3">
          <button className="btn btn-cancel-custom flex-grow-1" onClick={() => setShowModal(false)} disabled={isSubmitting}>
            Cancelar
          </button>
          <button className="btn btn-save-custom flex-grow-1" disabled={isSubmitting}>
            {isSubmitting ? 'Guardando...' : editingDeporte ? 'Guardar Cambios' : 'Crear Deporte'}
          </button>
        </div>
        </form>
      </div>
